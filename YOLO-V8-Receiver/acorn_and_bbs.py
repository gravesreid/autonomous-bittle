...
host_ip = '192.168.1.17'  # IP Address
    ...
    if frame is not None:
        ...
        if len(results) > 0: 
            ...
            for i, value in enumerate(y_list): # loop y values to find the minimum 
                if y_max < value: # change y_min to new minimum
                    y_max = value
                    max_index = i
            if result_list: # if result list isn't empty, there is at least one detection
                ...
                if result_list[0] == 0.0: # 0.0 is an acorn detection
                    ...
                    if x_list[max_index] < 0.25 and current_command != "kcrL": # block is to the left
                        print("sending crawl left command")
                        current_command = "kcrL"
                    elif x_list[max_index] > 0.75 and current_command != "kcrR": # block is to the right 
