...
class Sender:
    def __init__(self):
        ...
        pi_ip = '192.168.1.168' # raspberry pi ip address
        pi_port = 12346 # must match port in ardSocket.py on pi
        self.info_socket.connect((pi_ip, pi_port))
        
    def send_info(self, info):
        try:
            serialized_info = pickle.dumps(info)
            self.info_socket.sendall(serialized_info)
        except:
...
while True:
    ...
    if frame is not None:
        ...
        if len(results) > 0: 
            ...
            #added pickle sending of detection info
            info_dict = {"result_list": result_list, "xywhn_list": xywhn_list}
            sender.send_info(info_dict)
