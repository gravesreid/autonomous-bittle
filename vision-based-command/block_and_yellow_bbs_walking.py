    ...
    if frame is not None:
        ...
        if len(results) > 0: 
            ...
            if result_list: # if result list isn't empty, there is at least one detection
                ...
                if result_list[0] == 0.0: # 0.0 is a green block detection
                    # determine if green block is to left or right side of frame
                    if x_list[min_index] < 0.25 and current_command != "kwkL": # block is to the left
                        print("sending crawl left command")
                        current_command = "kwkL"
                        commander.send_command('\nkwkL')
                    elif x_list[min_index] > 0.75 and current_command != "kwkR": # block is to the right 
                        print("sending crawl right command")
                        current_command = "kwkR"
                        commander.send_command('\nkwkR')
                    elif current_command != "kwkF": # Block is in center 
                        print("sending crawl forward command")
                        current_command = "kwkF"
                        commander.send_command('\nkwkF')
                elif result_list[0] == 1.0: # 1.0 is a pheromone detection
                    # determine if pheromone is to left or right side of frame
                    if x_list[min_index] < 0.25 and current_command != "kwkL": # pheromone is to the left
                        print("sending crawl left command")
                        current_command = "kwkL"
                        commander.send_command('\nkwkL')
                    elif x_list[min_index] > 0.75 and current_command != "kwkR": # pheromone is to the right 
                        print("sending crawl right command")
                        current_command = "kwkR"
                        commander.send_command('\nkwkR')
                    elif current_command != "kwkF": # pheromone is in center 
                        print("sending crawl forward command")
                        current_command = "kwkF"
                        commander.send_command('\nkwkF')
            else:
                ...
                if pan_time<search_time and current_command != "kwkR":
                    current_command = "kwkR"
                    commander.send_command('\nkwkR')
                    print("pan time: ", pan_time)
                if pan_time>search_time and pan_time<(3*search_time) and current_command != "kwkL":
